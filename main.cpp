...
int get_vertices(){
    ...
    arquivo = fopen("verticesCG.csv", "r");
    if (arquivo == NULL)
    {
...
void le_arquivo (float  fVert[])
{
    FILE *arquivo;

    float*  fVertices = fVert;
    int n = 0;
    int i = 0;
    ...
    while (fscanf(arquivo, "%f", &fVertices[n++]) == 1)
    {
        fscanf(arquivo, ";");
    }
...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "OPenGL : Trabalho II", NULL, NULL);
    if (window == NULL)
    {
    ...
    int tam = get_vertices();
    float vertices2[tam];

    le_arquivo(vertices2);

    for(int j = 0 ; j < tam/8 ; j++){
            for(int i = j * 8 ; i < j*8+8 ; i++){
                printf("|%.1f|",vertices2[i]);
            }
        printf("\n");
    }
